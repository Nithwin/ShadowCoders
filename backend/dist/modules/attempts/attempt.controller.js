...
exports.startAttemptHandler = startAttemptHandler;
...
exports.submitAttemptHandler = submitAttemptHandler;
const getAttemptDetailsHandler = async (req, res, next) => {
    try {
        const studentId = req.user?.sub; // From verifyAccess middleware
        const attemptId = req.params.attemptId; // From URL parameter
        if (!studentId) {
...
exports.getAttemptDetailsHandler = getAttemptDetailsHandler;
...
const getAttemptResultsHandler = async (req, res, next) => {
    try {
        const studentId = req.user?.sub; // From verifyAccess middleware
        const attemptId = req.params.attemptId; // From URL parameter
        if (!studentId) {
...
const getStudentAttemptsHandler = async (req, res, next) => {
    try {
        const studentId = req.user?.sub; // From verifyAccess middleware
        if (!studentId) {
            return next({ status: 401, message: 'Unauthorized' });
...
exports.getStudentAttemptsHandler = getStudentAttemptsHandler;
const listAttemptsForExamHandler = async (req, res, next) => {
    try {
...
exports.listAttemptsForExamHandler = listAttemptsForExamHandler;
const getAttemptForAdminHandler = async (req, res, next) => {
    try {
...
exports.getAttemptForAdminHandler = getAttemptForAdminHandler;
const resetAttemptsHandler = async (req, res, next) => {
    try {
        // Get validated body data
        const resetData = req.validatedData?.body || req.body;
        if (!resetData.examId) {
