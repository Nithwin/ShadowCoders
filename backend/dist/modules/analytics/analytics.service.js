...
exports.getExamAnalytics = exports.getExamStatistics = exports.getTimeSpentAnalysis = exports.getStudentPerformanceTrends = exports.getQuestionPerformanceMetrics = exports.calculateDiscriminationIndex = exports.calculateQuestionDifficulty = void 0;
...
const calculateQuestionDifficulty = async (questionId) => {
    const responses = await prisma_1.prisma.response.findMany({
        where: {
            questionId,
            attempt: {
                status: 'SUBMITTED',
...
exports.calculateQuestionDifficulty = calculateQuestionDifficulty;
/**
 ...
 */
const calculateDiscriminationIndex = async (questionId) => {
    ...
    const calculateProportionCorrect = (group) => {
        let correct = 0;
        group.forEach((item) => {
...
exports.calculateDiscriminationIndex = calculateDiscriminationIndex;
/**
 * Get question performance metrics for an exam
 */
const getQuestionPerformanceMetrics = async (examId) => {
    ...
    const metrics = await Promise.all(questions.map(async (question) => {
        const responses = await prisma_1.prisma.response.findMany({
            where: {
        ...
        const difficulty = await (0, exports.calculateQuestionDifficulty)(question.id);
        const discriminationIndex = await (0, exports.calculateDiscriminationIndex)(question.id);
        return {
            questionId: question.id,
            questionOrder: question.order,
            questionPrompt: question.prompt?.substring(0, 100) || '',
...
exports.getQuestionPerformanceMetrics = getQuestionPerformanceMetrics;
/**
 * Get student performance trends over time for an exam
 */
const getStudentPerformanceTrends = async (examId) => {
    const attempts = await prisma_1.prisma.attempt.findMany({
        where: {
            examId,
            status: 'SUBMITTED',
...
exports.getStudentPerformanceTrends = getStudentPerformanceTrends;
/**
 * Get time spent analysis per question
 */
const getTimeSpentAnalysis = async (examId) => {
    const questions = await prisma_1.prisma.question.findMany({
        where: {
            examId,
            status: 'ACTIVE',
    ...
    const analysis = await Promise.all(questions.map(async (question) => {
        ...
        const median = timeSpent.length % 2 === 0
...
exports.getTimeSpentAnalysis = getTimeSpentAnalysis;
/**
...
const getExamStatistics = async (examId) => {
    ...
    const submittedAttempts = await prisma_1.prisma.attempt.findMany({
        where: {
            examId,
            status: 'SUBMITTED',
        },
    ...
    const allAttempts = await prisma_1.prisma.attempt.count({
        where: { examId },
    });
...
const getExamAnalytics = async (examId) => {
    const [statistics, questionMetrics, performanceTrends, timeAnalysis] = await Promise.all([
        (0, exports.getExamStatistics)(examId),
        (0, exports.getQuestionPerformanceMetrics)(examId),
        (0, exports.getStudentPerformanceTrends)(examId),
        (0, exports.getTimeSpentAnalysis)(examId),
    ]);
    return {
        statistics,
        questionMetrics,
...
exports.getExamAnalytics = getExamAnalytics;
