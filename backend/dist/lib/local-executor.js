...
exports.executeCodeLocally = executeCodeLocally;
exports.testCodeWithTestCasesLocally = testCodeWithTestCasesLocally;
 ...
 */
async function executeWithTimeout(command, input, cwd, timeoutMs) {
 ...
 */
async function testCodeWithTestCasesLocally(code, language, testCases) {
    const langConfig = LANGUAGE_CONFIGS[language.toLowerCase()];
    ...
    const results = await Promise.all(testCases.map(async (testCase) => {
        try {
            const response = await executeCodeLocally(code, language, testCase.input, testCase.timeoutMs || 5000);
 ...
 */
async function testCompiledCodeWithTestCases(code, language, testCases) {
    const langConfig = LANGUAGE_CONFIGS[language.toLowerCase()];
    if (!langConfig) {
    ...
    try {
        ...
        const results = await Promise.all(testCases.map(async (testCase, idx) => {
            try {
                // Build run command (override for Java to handle package + classpath root)
                let runCmd = langConfig.runCommand(filePath);
                if (language.toLowerCase() === 'java') {
...
exports.SUPPORTED_LANGUAGES = Object.keys(LANGUAGE_CONFIGS);
