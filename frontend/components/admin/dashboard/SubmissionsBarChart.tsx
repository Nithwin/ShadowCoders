...
/* custom tooltip */
function CustomTooltip({ active, payload, label, resolvedTheme }: any) {
  ...
  if (data.length === 0) {
    return (
      <div className="h-[280px] flex flex-col items-center justify-center text-gray-400 dark:text-slate-400">
        <BarChart3 className="w-10 h-10 text-gray-200 dark:text-slate-700 mb-3" />
        <p className="text-sm font-medium">No submissions yet</p>
      </div>
    );
  }

  const gridStroke = resolvedTheme === 'dark' ? '#334155' : '#e5e7eb';
  const axisColor = resolvedTheme === 'dark' ? '#94a3b8' : '#94a3b8';
  const cursorColor = resolvedTheme === 'dark' ? 'rgba(59,130,246,0.12)' : 'rgba(37,99,235,0.04)';
    ...
    <ResponsiveContainer width="100%" height={280}>
      <BarChart data={data} barCategoryGap="25%">
        ...
        <YAxis
          ...
          allowDecimals={false}
        />
        <Tooltip content={<CustomTooltip resolvedTheme={resolvedTheme} />} cursor={{ fill: cursorColor, radius: 6 }} />
